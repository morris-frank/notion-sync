...
export function markdownToRichText(markdown: string): RichText[] {
  ...
  for (const match of markdown.matchAll(pattern)) {
    ...
    else if (token.startsWith("**") || token.startsWith("__")) result.push(textObject(token.slice(2, -2), { bold: true }));
...
const CODE_LANGUAGES = new Set([
  ...
  "diff", "docker", "elixir", "elm", "erlang", "flow", "fortran", "f#", "gherkin", "glsl", "go", "graphql",
  "groovy", "haskell", "html", "java", "javascript", "json", "julia", "kotlin", "latex", "less", "lisp", "lua",
  ...
  "scheme", "scss", "shell", "sql", "swift", "typescript", "vb.net", "verilog", "vhdl", "visual basic", "webassembly",
  "xml", "yaml", "java/c/c++/c#"
]);
const CODE_LANGUAGE_ALIASES: Record<string, string> = {
  "": "plain text", text: "plain text", txt: "plain text", js: "javascript", ts: "typescript", py: "python",
  rb: "ruby", sh: "shell", zsh: "shell", yml: "yaml", md: "markdown", tex: "latex", cpp: "c++", cs: "c#"
};
...
export function markdownToBlocks(markdown: string): NotionBlock[] {
  ...
  for (let index = 0; index < lines.length;) {
    ...
    if (callout) {
      flush(); blocks.push(callout.block); index = callout.end; continue;
    }
    ...
    if (heading) { flush(); blocks.push(richTextBlock(`heading_${heading[1]?.length}`, heading[2] ?? "")); index += 1; continue; }
    if (/^\s*(---|\*\s*\*\s*\*)\s*$/.test(line)) { flush(); blocks.push({ object: "block", type: "divider", divider: {} }); index += 1; continue; }
    const todo = line.match(/^\s*[-*]\s+\[([ xX])\]\s+(.+)$/);
    if (todo) { flush(); blocks.push(richTextBlock("to_do", todo[2] ?? "", { checked: todo[1]?.toLowerCase() === "x" })); index += 1; continue; }
    const bullet = line.match(/^\s*[-*+]\s+(.+)$/);
    if (bullet) { flush(); blocks.push(richTextBlock("bulleted_list_item", bullet[1] ?? "")); index += 1; continue; }
    const numbered = line.match(/^\s*\d+[.)]\s+(.+)$/);
    if (numbered) { flush(); blocks.push(richTextBlock("numbered_list_item", numbered[1] ?? "")); index += 1; continue; }
    const quote = line.match(/^>\s?(.*)$/);
    if (quote) { flush(); blocks.push(richTextBlock("quote", quote[1] ?? "")); index += 1; continue; }
    paragraph.push(line);
    index += 1;
  }
  flush();
...
export const SAFE_REPLACE_BLOCK_TYPES = new Set([
  ...
  "numbered_list_item", "to_do", "quote", "code", "callout", "divider"
]);
...
export function validateWritableBlocks(blocks: NotionBlock[]): void {
  const visit = (block: NotionBlock): void => {
    if (!SAFE_REPLACE_BLOCK_TYPES.has(block.type)) throw new Error(`Cannot write unsupported block type: ${block.type}`);
    const richText = blockData(block).rich_text;
    if (Array.isArray(richText)) {
      if (richText.length > 100) throw new Error(`A ${block.type} block contains more than 100 rich-text spans; split it into smaller blocks`);
      for (const item of richText as RichText[]) {
        if ((item.text?.content ?? "").length > 2000) throw new Error(`A ${block.type} rich-text span exceeds Notion's 2,000-character limit`);
      }
    }
...
export function blocksToMarkdown(blocks: NotionBlock[]): string {
  ...
  for (const block of blocks) {
    ...
    switch (block.type) {
      case "paragraph": output.push(text); break;
      case "heading_1": output.push(`# ${text}`); break;
      case "heading_2": output.push(`## ${text}`); break;
      case "heading_3": output.push(`### ${text}`); break;
      case "bulleted_list_item": output.push(`- ${text}`); break;
      case "numbered_list_item": output.push(`1. ${text}`); break;
      case "to_do": output.push(`- [${data.checked ? "x" : " "}] ${text}`); break;
      case "quote": output.push(`> ${text}`); break;
      case "divider": output.push("---"); break;
      case "code": output.push([`\`\`\`${String(data.language ?? "")}`, text, "```"].join("\n")); break;
      case "callout": {
      ...
      default: output.push(`<!-- notion-sync: unsupported ${block.type} block omitted -->`);
    }
    if (block.children?.length) output.push(...blocksToMarkdown(block.children).split("\n").map((line) => `  ${line}`));
  }

...
describe("Notion API write sequencing", () => {
  ...
  it("creates a page under the configured data source", async () => {
    requestUrlMock.mockResolvedValueOnce(ok({ id: "page-1", last_edited_time: "2026-07-19T10:00:00Z", properties: {} }) as never);
    ...
    expect(requestUrlMock).toHaveBeenCalledWith(expect.objectContaining({
      ...
      body: JSON.stringify({ parent: { type: "data_source_id", data_source_id: "source-1" }, properties: { Name: { title: [] } } })
    }));
  });
  ...
  it("chunks appends at the API's 100-block boundary", async () => {
    ...
    const bodies = requestUrlMock.mock.calls.map(([request]) => JSON.parse((request as { body: string }).body) as { children: unknown[] });
  ...
  it("appends the replacement before trashing old blocks", async () => {
    ...
    await new NotionApi("secret").replacePage(
      "page-1", { Name: { title: [] } }, [{ type: "paragraph", paragraph: { rich_text: [] } }], ["old-1", "old-2"]
    );
    expect(requestUrlMock.mock.calls.map(([request]) => `${(request as { method: string }).method} ${(request as { url: string }).url}`)).toEqual([
